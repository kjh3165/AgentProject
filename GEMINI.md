...
* **최소 변경 원칙:** 사용자가 명시적으로 요청한 기능이나 수정 범위에만 집중한다. 기능 구현을 위해 반드시 연관된 코드가 아닌 이상, 기존의 스타일이나 로직을 임의로 리팩토링하거나 변경하지 않는다.
* **제안 후 수정:** 명시적 요청 외에 추가 수정이 필요하다고 판단되는 경우, 임의로 수정하지 않고 사용자에게 해당 내용과 이유를 간단히 설명한 뒤 승인을 받아 진행한다.
...
* **Git 도구 사용:** Git 관련 명령어는 PowerShell이 아니라 Git Bash를 사용하여 실행한다.
* **Git 명령어 단일 실행:** `&&` 등을 사용하여 여러 명령어를 혼합하지 않고, 한 번에 하나의 Git 명령어만 실행한다.
* **브랜치 활용:** 새로운 기능 개발이나 코드 수정은 반드시 `main` 브랜치가 아닌 전용 작업 브랜치(예: `feat/xxx`, `fix/xxx`)에서 진행한다.
* **문서 업데이트 시점:** 기능 개발 완료 직후가 아니라, 사용자가 직접 테스트를 수행하고 승인한 후에 관련 문서(TODO, CONTEXT 등)를 업데이트한다.
* **사용자 확인 필수 (중요):**
    *   기능 개발이나 수정이 완료된 후, GitHub 원격 저장소로 **푸시(Push)하기 직전**에 반드시 사용자에게 확인을 받는다.
    *   **보고 시점 준수:** 모든 도구 실행(특히 Git Push)에 대한 완료 보고는, 실제 명령어 승인 대화창이 닫히고 도구로부터 성공 결과가 반환된 것을 확인한 후에만 수행한다.
    *   작업 브랜치에서의 개발이 완료되어 `main` 브랜치로 **통합(Merge/Reset)하기 직전**에 반드시 사용자에게 확인을 받는다.
    *   `main` 브랜치의 내용을 GitHub 원격 저장소로 **푸시(Push)하기 직전**에 반드시 사용자에게 확인을 받는다.
* **히스토리 관리:** `main` 브랜치로 통합할 때는 불필요한 머지 커밋 없이 깔끔한 한 줄(Fast-forward) 히스토리가 되도록 관리한다.
